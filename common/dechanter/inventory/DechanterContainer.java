...
public class DechanterContainer extends Container {
	...
	public DechanterContainer(TileDechanter theTile, InventoryPlayer inventory) {
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer,
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotIndex < numRows * 9) {
				if (!this.mergeItemStack(stackPlaceHolder, numRows * 9,
						inventorySlots.size(), true))
					return null;
			} else if (!this.mergeItemStack(stackPlaceHolder, 0, numRows * 9,
					false))
				return null;
